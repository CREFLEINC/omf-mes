...
CONTRACT = os.path.join(ROOT, 'uiux', '화면상세스펙-공통', '공유계약.md')
DSGAP = os.path.join(ROOT, 'uiux', '화면상세스펙-공통', 'ds-gap.md')
UIUX = os.path.join(ROOT, 'uiux')

# ── 조항처럼 보이지만 조항이 아닌 것 — 실측으로 확인한 세 갈래.
#
#   I-N   §I 「확정 ↔ 하류 불일치 대장」 항목 (실측: `### I-N.` 헤딩 0건)
#   E-N   고객 회신 번호와 §E 조항(E-1~E-4)이 같은 모양이다. 회신이 E-14 까지
#         있어 겹친다 → **E-3 이상은 회신으로 본다**(그래서 E-3·E-4 조항은 미검사)
