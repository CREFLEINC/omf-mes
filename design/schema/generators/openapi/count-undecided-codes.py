...
이 스크립트가 내는 수는 **기계적 후보**다 — 「계약에서 이런 모양으로 나타나는 코드
필드가 몇이나 되나」까지이고, 「그중 무엇이 아직 안 정해졌나」는 **아니다**. 문서가
수치를 인용할 때 이 출력을 붙이되 **후보 수라고 적는다.**

⛔ 판정의 정본은 여기가 아니다 — 2026-09-03 뒤집혔다
------------------------------------------------------
`design/wiki/progress/99-설계진도대장.md` 가 이 스크립트의 출력을 「확정되지 않은 업무
코드 47종」으로 실었는데, 실측하니 **46종은 이미 판정이 끝나 있었다** —
사전이 값을 갖는 것이 39, 계약이 「코드 그룹이 아니다」로 판정한 것이 7. 최종
인도물이 거짓을 말한 것이다. 뿌리는 **기계적 후보를 판정 결과로 읽은 것**이다.

    기계적 후보   이 스크립트                              — 「어떤 모양인가」
    판정 정본     `design/schema/code-dictionary.md`        — 「값이 정해졌나」
                  `openapi/check-code-dictionary.py`       — 사전과 계약의 대조

⇒ 「정해야 할 것이 몇 종인가」를 물으면 **코드 사전과 그 검사기**에 묻는다. 이 수를
그대로 옮겨 적지 않는다.
...
- ⭐ **업무 코드** — 나머지. **값 목록을 정할 «후보»다 — 「아직 안 정해졌다」가
  아니다.** 실제로 정해졌는지는 코드 사전이 말한다(위 ⛔ 절)
"""
...
def main() -> int:
    ...
    print(f"        └ ⭐ 업무 코드              {len(business)}   ← 값 목록 «후보»")
    print("        ⛔ 후보는 판정이 아니다 — 값이 정해졌는지는 code-dictionary.md 가 말한다")
    if partial:
        print()
