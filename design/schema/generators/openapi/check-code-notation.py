...
------------------------------------------------------------
실측(2026-09-03) — 계약 7벌 산문에 병기 **91자리**(고유 36짝)인데 그중 상당수는
병기를 빼면 **오독한다**:
...
def paired(text: str, value: str) -> tuple[int, int]:
    ...
    병기 = 그 값 바로 뒤에 «한국어를 담은 괄호»가 온다. backtick 은 있어도 없어도 된다.
    """
    both = solo = 0
    for m in re.finditer(r"`?\b%s\b`?" % re.escape(value), text):
