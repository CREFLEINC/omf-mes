...
#
# ⚠ 20 → 26 (2026-09-03 · 같은 날 두 번째). ⛔ **같은 구멍을 반복한 것이 아니다** —
#    계약의 「맨몸」 자리 33곳에 `codeGroupCode=` 포인터를 «적자»(④ 축 정리) 그 자리가
#    가리키는 그룹이 처음으로 보이게 됐고, 그 테이블을 쓰는 형제 화면 8건이 새로
#    대상이 됐다. **검사가 넓어져 가려져 있던 자리가 드러난 것**이고, 같은 형태를
#    이 파일이 이미 한 번 겪었다(`screen_sections()` 결함을 고치자 58 → 59).
#    ⭐ 그중 **2건은 진짜 결손이라 닫았다** — `P-01-01`·`W-02-04` 는 `lot_type_code` 를
#    §4 에 «실제로 갖고 있는데» 요구서에 `codeGroupCode=LOT_TYPE` 호출이 없었다.
#    남은 6 은 「그 칸이 §4 에 없다」 — 형제가 그 테이블을 쓸 뿐이다.
#
...
#
# ⭐⭐ 26 → 20 (2026-09-03 · 전건 판정). 26건을 **화면마다 §3 목업·§4·§5·§8 과
#    형제 요구서까지 대조해** 「그 칸을 이 화면이 보여야 하는가」를 가렸다. 결과:
#
#    ✅ 닫은 6 — 「보여야 한다」
#      · `M-01-04`·`W-04-08` × `LOT_STATUS` — 잔고 행이 «위치별»이라 품질 상태가
#        행을 나눈다(`M-01-04` §5-1 이 세운 노출 기준 「그 축이 응답의 행을
#        나누는가」). ⭐ `M-01-04` §8-5 는 2026-09-02 에 이미 「✅ 해소 — 품질
#        `LOT_STATUS`」로 적어 두었는데 §4·요구서가 안 따라갔다.
#      · `W-05-12` × `CYCLE_TYPE` — ⚠ **결론은 맞는데 검사기의 «이유»는 헛짚었다.**
#        걸린 칸은 `mdm.equipment.calibration_cycle_type_code`(검교정 · `W-05-11`
#        소관)인데, 진짜 결손은 §4-C-2 «점검 부여» 주기다(계약
#        `InspectionItemAssignment.cycleTypeCode`). 요구서가 그 PUT 을 이 화면
#        몫으로 이미 적었고 §7 DS 매핑이 「주기 기준일」 입력을 걸어 두었다.
#      · `P-01-01`·`W-02-04` × `LOT_TYPE` — 위 회차에서 닫았다.
#
#    ⛔ 헛짚음으로 판정한 것들 — «다시 파지 마라». 근거가 스펙 안에 있다:
#      · `W-03-03` × 해제 사유 — §4-A 가 **명문으로 배제**했다(「해제 컬럼은 이
#        화면이 쓰지 않는다 — `W-03-02` 소관」).
#      · `W-04-08`·`W-01-07`·`M-01-04` × 해제 사유 — ⛔ **계약이 못 준다.**
#        `GET /trace/lots/{lotId}` 가 「**해제되지 않은** 보류」만 내린다.
#      · `P-02-07` × `LOT_STATUS` — 목업의 「상태」 값이 `완료`·`미달` 이라 «다른
#        축»(`LOT_LIFECYCLE_STATUS`)이고 **그 호출은 이미 실려 있다**.
#      · `P-02-06`·`M-04-04`·`P-02-07`·`W-04-08` × `LOT_TYPE` — 화면이 **단일
#        유형만** 다룬다(생산LOT만 · 제품LOT만 · 완제품만). 열을 두면 전 행이 같다.
#      · `W-03-02` × `LOT_TYPE` — §4-A 소절 제목이 「**`status_code` 한 컬럼**」으로
#        범위를 명시적으로 좁혔다. 유형으로 거르는 일은 앞 화면(`W-03-01`)이 한다.
#      · `P-05-01` × `CYCLE_TYPE` — §4-B 가 읽는 칸을 **4개로 명시 열거**했고 이
#        화면의 축은 «타수» 하나다. 날짜 주기 PM 은 `W-05-02` 소관.
#      · `M-04-04` × `GOODS_RECEIPT_REASON` — §4-B 가 `receipt_type_code` 를
#        「**제품입고 고정**」으로 못박아 사유가 필요한 갈래(반품)와 갈라 놓았다.
#        형제 `W-04-06` 이 「이 화면이 주 사용처다」로 소유를 선언했다.
#      · `P-04-04` × `HANDLING_UNIT_TYPE` — 발번·인쇄 «전용»이라 포장을 만들지
#        않는다. ⚠ 목업의 「유형」 열은 «분할/합병»(재구성 이벤트)이라 다른 축이다.
#      · `W-01-03` × `SUBSTITUTE_LOT_REASON` — 이 화면은 «입력하지 않는다».
#        대체 LOT 판단은 `M-01-01` 에서 끝났고 §4-B 는 「어느 값이 갈리는가」
#        대조표다.
#      · `M-01-04`·`W-04-08` × `lot_hold.lot_status_code` — 계약 «파생» 칸이고
#        (`x-source-column` 이 없다) 이 칸을 보이라는 문장이 7화면 어디에도 없다.
#
#    ⏸ 판정 보류 2 — `W-01-10`·`W-04-08` × `OWNERSHIP_TYPE`. 「완제품 창고·자재
#      입고에 자사 외 소유(사급·수탁)가 실제로 서는가」는 **업무 사실**이라
#      스펙으로 못 가른다. `M-01-04` 가 세운 노출 기준에는 걸리지만, 그 창고에
#      그런 재고가 없으면 열이 늘 비어 있다.
#
#    ⏸ 계약 선행 1 — `W-03-01` × 해제 사유. ⭐ **목업이 칸을 그려 놓고 비워 뒀다**
#      (이력 표 「사유」 열의 해제 행이 `—`)이고 §5-1 도 「합격이라서가 안 남는다」를
#      결손으로 적었다. 다만 이 화면이 부르는 `GET /quality/lot-hold-events` 의
#      `LotHoldEvent` 에 `releaseReasonCode` 가 **없다** — 계약 신설이 선행이다.
BASELINE_SCREEN = 20
