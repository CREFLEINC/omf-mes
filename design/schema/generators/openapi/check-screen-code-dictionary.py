...
#
# ⭐ 4 → 1 (2026-09-03). `P-02-03` 3줄이 «미결이 아니었다»는 판정으로 닫혔다.
#    앞선 주석은 「계약에 프로퍼티 자체가 없으니 판정 전에 이름을 붙이라는 말이 된다」로
#    읽었는데, ⛔ **그 읽기가 틀렸다.** 없는 것은 «이름»이 아니라 «이 화면의 소관»이다:
#      · §4-C 는 「투입 확정 시 «서버»가 만드는 계보」를 적은 **설명 표**다
#      · 화면이 보내는 `MaterialConsumptionCreate` 에 계보 칸이 **0 개**
#      · 계약에 계보를 «내리는» 응답도 **0 개**
#    ⇒ 화면이 보내지도 받지도 않으니 값 목록은 이 화면의 미결이 아니다
#      (`A-21` 「자리가 있다고 채우지 않는다」). 실은 그 화면 자신이 §5-4 에
#      「화면은 이 값을 보내지 않는다」, §8-4 에 「판정 주체는 서버다」를 이미
#      적어 두었고 — **문면 세 자리가 그 판정을 안 따라간 것뿐이었다.**
#
# ⚠ 남은 1 은 고치면 «안 되는» 자리다.
#    · `P-04-02` 1줄 — 「②로 물러난 이유」의 «인용문»이고 바로 두 줄 뒤가 스스로
#      반박한다. 고치면 반박 대상이 사라진다. ⛔ 작성 규칙 5 의 표식을 달 수도 있으나
#      인용이 두 줄에 걸쳐 있어 표식이 문장 «중간»에 박힌다 — 문장을 비틀어 검사기를
#      맞추지 않는다.
