...
⭐ **「진짜 결손」은 0이다.** 뜻은 확정인데 코드 문자열이 없던 마지막 하나가
`CD-APP-USER-STATUS` 였고, `§G` 규약대로 영문 SNAKE 로 옮겨 채웠다
(`EMPLOYED` `ON_LEAVE` `RESIGNED` · `omf-mes#408` · 2026-09-03).
«(구표기 보존: ~~나머지 하나는 진짜 결손이다 — `CD-APP-USER-STATUS` 는 뜻은 확정인데
...
있다」는 표시다(`registry` 9 · `registry-system` 1). 그 값 목록이 완전한가는 «소유» 열이
말한다(위 표).
...
| `CD-DASHBOARD-CARD` | `PRODUCTION_QTY` `YIELD_RATE` `PERFORMANCE_RATE` `AVAILABILITY_RATE` `UNHANDLED_ALARM` `OEE` | — | `cardCode` | `enum` | 1 | 계약이 `enum` 으로 닫은 값 — `DashboardCard`(`app-공통`). ⭐ 화면이 이 코드로 «드릴다운 대상»을 안다 — 각 카드가 자기 소유 화면으로 간다(`W-CO-05` §5). ⛔ 카드가 늘면 드릴다운 대상도 함께 정해야 한다 |
...
| `CD-DEFECT-RESPONSIBILITY-TYPE` | ⬜ | `DEFECT_RESPONSIBILITY_TYPE` | `responsibilityTypeCode` | `registry` | 1 | 귀책 구분. ⚠ «원천 축이 아니다» — 어디서 «발견»했나(`sourceCode`)와 다르다(`W-03-05` §5-2) |
...
| `CD-INSPECTION-FREQUENCY-INTERVAL-UOM` | `HOUR` `QUANTITY` | `INSPECTION_FREQUENCY_INTERVAL_UOM` | `frequencyIntervalUomCode` | `registry-system` | 3 | 검사 주기의 «단위». ⭐ 두 축은 사용자와 협의해 확정했다(2026-09-03). ⛔ 기준단위(`uomCode`)와 다른 축이다 |
...
| `CD-INSPECTION-REQUEST-TARGET-TYPE` | `LOT` `WORK_ORDER` `SHIPMENT_REQUEST` | — | `targetTypeCode` | `enum` | 1 | 계약이 `enum` 으로 닫은 값 — `InspectionRequest`(`quality-03품질`). ⭐ `W-04-03` §4 「다형 — 출하작업지시 · LOT」 · `P-02-13` §4(작업지시)에서 도출. ⚠ `lotId`·`workOrderId`·`productionResultId` 와 병존한다 — `A-10` 규칙 3 대로 같은 대상이면 FK 쪽을 쓴다 |
...
| `CD-INTERFACE-CONNECTION-FAILURE-CAUSE` | `UNREACHABLE` `TABLE_NOT_FOUND` `PERMISSION_DENIED` | — | `failureCauseCode` | `enum` | 1 | 계약이 `enum` 으로 닫은 값 — `InterfaceConnectionTestResult`(`mdm-기준정보`). ⭐ 「연결 실패」 한 마디로 뭉치면 무엇을 고쳐야 할지 알 수 없다 |
| `CD-INTERFACE-TARGET` | `ITEM` `BOM` `ORGANIZATION` `WORKER` `PURCHASE_ORDER` ⬜ | `INTERFACE_TARGET` | `targetCode` | `registry` | 6 | 연계 대상. ⭐ 계약이 «일부러 열어 둔» 자리다 — 그 밖의 값도 받고 막지 않으며 확정 목록 안인지는 `withinConfirmedScope` 가 말한다 |
| `CD-INVENTORY-ADJUSTMENT-REASON` | `COUNT_VARIANCE` `TRANSPORT_DAMAGE` `HOPPER_MEASUREMENT` `SYSTEM_ERROR_CORRECTION` `OTHER` | `INVENTORY_ADJUSTMENT_REASON` | `reasonCode` | `registry` | 5 | 재고조정 사유. `omf-mes#198` 시드(`design/raw/…/2026-08-13-공통코드값목록-제안안`) |
...
| `CD-INVENTORY-RESERVATION-SOURCE-DOCUMENT-TYPE` | `PRODUCTION_ORDER` · `SHIPMENT_REQUEST_LINE` | — | `sourceDocumentTypeCode` | `enum` | 1 | 계약이 `enum` 으로 닫은 다형 참조 판별자 — `InventoryReservation`(`logistics-01자재창고`) **한 자리**. `PRODUCTION_ORDER`는 생산오더, `SHIPMENT_REQUEST_LINE`은 제품 피킹의 출하요청 라인을 가리킨다. 이 자원은 **조회만 제공한다**(예약은 출고 요청·피킹의 결과로 서버가 걸고 푼다 · `M-01-08` §5-5). 일반 예약 조회는 아직 원천 유형 조건을 받지 않으므로 유형별 조회 지원과 응답 값 목록을 혼동하지 않는다(`omf-mes#595`) |
| `CD-INVENTORY-TRANSACTION-SOURCE-DOCUMENT-TYPE` | `GOODS_RECEIPT` `GOODS_ISSUE` `INVENTORY_ADJUSTMENT` `STOCK_TRANSFER` `RECYCLE_ENTRY` `PUTAWAY_TASK` | — | `sourceDocumentTypeCode` | `enum` | 2 | 계약이 `enum` 으로 닫은 값 — `InventoryTransaction` 응답과 수불 원장 목록의 원천 유형 필터(`logistics-01자재창고`). 원장의 성격을 가르며 방향은 라인의 `from*`/`to*`가 나타낸다. `RECYCLE_ENTRY`는 재생재 등록 건의 `recycleEntryId`를 가리킨다(재생재 원장 검토 요청 `omf-mes#512` 사용자 승인 · `M-01-12` §5-2-a). `PUTAWAY_TASK`는 신규 임시 적치·복귀 원장의 `putawayTaskId`다(#454). 과거 원장 재작성·취소 유형 추가는 없다. 기존 `STOCK_TRANSFER`는 재고 이동의 원장 기록에 따른 판정이다 |
...
| `CD-LOGISTICS-DOCUMENT-STATUS` | `REGISTERED` `POSTED` `CANCEL_REQUESTED` `CANCELLED` | `LOGISTICS_DOCUMENT_STATUS` | `statusCode` | `registry-system` | 22 | 물류 전표 9종 공용. ⭐ `W-01-13` §3 목업이 네 값을 이미 그렸다 — 꺼낸 것이지 정한 것이 아니다. 9종은 `CD-LOGISTICS-DOCUMENT-TYPE` 과 같은 집합이다 · 사용자 결정 2026-09-02 |
...
| `CD-MAINTENANCE-INSPECTION-RESULT` | `PASS` `FAIL` | — | `overallResultCode` | `enum` | 4 | 계약이 `enum` 으로 닫은 값 — `/maintenance/inspections`(`equipment-05설비툴`). ⭐ 문자열 근거는 «같은 파일의 일관성» — `Calibration.resultCode` 가 이미 `PASS`·`FAIL` 을 쓴다. 「`OK`」는 저장소 다른 곳에 용례가 없다 |
...
| `CD-MAINTENANCE-ORDER-TRIGGER-TYPE` | `BREAKDOWN` `INSPECTION_NG` `PM_DUE` | `MAINTENANCE_ORDER_TRIGGER_TYPE` | `triggerTypeCode` | `registry-system` | 1 | 보전 지시를 촉발한 것. ⛔ 연계 정의의 `CD-TRIGGER-TYPE`(`EVENT`·`TIME_SCHEDULE`)과 이름만 같고 축이 다르다. ⭐ 이 «조합»이 `maintenanceTypeCode` 를 정한다 — 고장이 하나라도 섞이면 사후로 고정된다(`W-05-05` §5-3). ⛔ 시스템 소유 — 값이 늘면 그 판정 규칙이 무너진다 |
| `CD-MAINTENANCE-RESULT-LINE-RESULT` | ⬜ | `MAINTENANCE_RESULT_LINE_RESULT` | `resultCode` | `registry` | 1 | 보전 실적의 항목·부위별 결과. ⚠ 값 집합이 «대상에 따라 다르다» — 설비 보전과 툴 예방보전이 다르다. ⛔ 같은 이름 셋째 갈래다(`B-28`) |
...
| `CD-MAINTENANCE-TYPE` | `CORRECTIVE` `PREVENTIVE` | — | `maintenanceTypeCode` | `enum` | 2 | 계약이 `enum` 으로 닫은 값 — `/maintenance/orders`(`equipment-05설비툴`). ⭐ 「예지(`PREDICTIVE`)」를 두지 않는다(사용자 결정 2026-09-02) — 촉발할 트리거가 아직 없다 |
| `CD-MANAGEMENT-LEVEL` | `WAREHOUSE` `ZONE` `RACK` `CELL` | `MANAGEMENT_LEVEL` | `managementLevelCode` | `registry-system` | 4 | 계약 `description` 산문에 이미 적혀 있던 값을 꺼냈다 |
...
| `CD-NONCONFORMANCE-STATUS` | `NOT_REQUESTED` `PENDING_DECISION` `DECIDED` | `NONCONFORMANCE_STATUS` | `statusCode` | `registry-system` | 3 | 부적합 의뢰·판정 축 |
| `CD-NOTIFICATION-RECIPIENT-TYPE` | `ROLE` `USER` | — | `recipientTypeCode` | `enum` | 1 | 계약이 `enum` 으로 닫은 값 — `NotificationRecipient`(`app-공통`). ⛔ 값이 «어느 짝 필드를 채우는지»를 가른다 — `ROLE` 이면 `businessUnitId`+`roleId`, `USER` 면 `userId` 다 |
| `CD-NOTIFICATION-TARGET-TYPE` | `EQUIPMENT` `MOLD` `INSTRUMENT` `PURCHASE_ORDER` `INTEGRATION_SYNC` `APPROVAL_REQUEST` `LOT` `WORK_ORDER` `NONCONFORMANCE` | — | `targetTypeCode` | `enum` | 1 | 계약이 `enum` 으로 닫은 값 — `Notification`(`app-공통`). ⛔ **2026-09-03 정정** — 전에는 `registry-system` 으로 등록부에 올라 있었는데 `A-16` 위반이다. ⚠ `LOT`·`WORK_ORDER`·`NONCONFORMANCE` 는 대응 이벤트가 알림센터 목록에 아직 없다 |
| `CD-NUMBER-SOURCE` | `MES` `SUPPLIER` | — | `numberSourceCode` | `enum` | 1 | 계약이 `enum` 으로 닫은 값 — `LotCreate`(`logistics-01자재창고`) |
| `CD-OPERATION-POLICY-MATCHED-SCOPE` | `ITEM` `PROCESS` `PLANT` `BUSINESS_UNIT` `ALL` | — | `matchedScopeCode` | `enum` | 1 | 계약이 `enum` 으로 닫은 값 — `OperationPolicyEffective`(`app-공통`). ⭐ 서버가 파생한다 — 이긴 정책이 «어느 축»으로 맞았는지. 값이 늘면 판정 «순서»가 함께 늘어야 한다 |
...
| `CD-PM-DUE-AXIS` | `SHOT` `DATE` | — | `pmDueAxisCode` | `enum` | 2 | 계약이 `enum` 으로 닫은 값 — `MaintenanceOrderTrigger`(`equipment-05설비툴`) · `Mold`(`mdm-기준정보`). ⭐ 서버가 파생한다 — `pmTriggerTypeCode` 가 「무엇으로 판정할지」라면 이 칸은 「판정축」이다. 단일 축 도래는 해당 축, 양축 도래·선후 확인은 먼저 도달한 축, 동시·선후 불명은 SHOT 대표축이다. 실제 타발수 선행을 단정하지 않는다. 미도래·해당 없음은 필드 생략(null 불가), 발행 시 재판정 결과를 스냅샷으로 보존한다. 근거: `W-05-02` §5-2 |
...
| `CD-SHIPPING-INSPECTION-STATUS` | `HELD` `NOT_REQUIRED` `PASSED` `PENDING` `REJECTED` | — | `shippingInspectionStatusCode` | `enum` | 2 | 계약이 `enum` 으로 닫은 값 — `ShipmentRequest` · `ShipmentLotAllocation`(`shipment-04제품출하`). 통합 출하 실적 화면은 배분 단위 상태를 표시하고 `oqcPassed` 파생 게이트로 납품 라벨 발행을 제어한다(P-04-01). |
...
| `CD-STOCK-REINSTATEMENT-REASON` | ⬜ | `STOCK_REINSTATEMENT_REASON` | `reasonCode` | `registry` | 1 | 재고 재등록 사유. ⭐ 「재등록이 몇 건인가」를 세려면 자유 텍스트가 아니라 코드 축이어야 한다(§I-41 · `omf-mes#84`) |
...
| `CD-TERMINAL-TYPE` | `POP` `MOBILE` | `TERMINAL_TYPE` | `terminalTypeCode` | `registry-system` | 4 | 단말 폼팩터. ⭐ 결정 16 이 확정한 아키텍처 축이다 — 고정 스테이션(POP)과 손에 드는 기기(MOBILE). 미결 대장이 「`terminal_type_code` 에 `MOBILE` 값이 있어야 한다」(`#64`)로 지목한 자리 |
| `CD-TOOL-TYPE` | `MOLD` `JIG` `OTHER` ⬜ | `TOOL_TYPE` | `toolTypeCode` | `registry` | 4 | 도구 유형. ⭐ `W-05-13` §3-3 이 「금형 / 지그 / 그 밖의 도구」로 확정했다 — `REQ-PR-0003` 「모든 도구」를 담는 축이고 테이블 이름(`mdm.mold`)은 바꾸지 않는다 |
...
| `CD-WORK-SESSION-EVENT-REASON` | `URGENT_ORDER_INTERRUPT` `EQUIPMENT_FAILURE` `TOOL_FAILURE` `MATERIAL_SHORTAGE` `MOLD_CHANGE` `QUALITY_ISSUE` `OTHER` | `WORK_SESSION_EVENT_REASON` | `reasonCode` | `registry` | 3 | 작업세션 이벤트 사유. `omf-mes#198` 시드(`design/raw/…/2026-08-13-공통코드값목록-제안안`). ⭐ **2026-09-06 게이트 승인** — `WorkOrderHold.reasonCode`(작업지시 중단·재개의 W/O 층 사유)도 이 그룹으로 통일됐다 — 값이 같고 `CD-WORK-ORDER-HOLD-REASON` 은 값 집합이 원래 비어 있었다(위 행 참조) |
