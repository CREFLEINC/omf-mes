...
| `CD-INVENTORY-TRANSACTION-SOURCE-DOCUMENT-TYPE` | `GOODS_RECEIPT` `GOODS_ISSUE` `INVENTORY_ADJUSTMENT` `STOCK_TRANSFER` `RECYCLE_ENTRY` | — | `sourceDocumentTypeCode` | `enum` | 2 | 계약이 `enum` 으로 닫은 값 — `InventoryTransaction` 응답과 수불 원장 목록의 원천 유형 필터(`logistics-01자재창고`). 원장의 성격을 가르며 방향은 라인의 `from*`/`to*`가 나타낸다. `RECYCLE_ENTRY`는 재생재 등록 건의 `recycleEntryId`를 가리킨다(재생재 원장 검토 요청 `omf-mes#512` 사용자 승인 · `M-01-12` §5-2-a). 기존 `STOCK_TRANSFER`는 재고 이동의 원장 기록에 따른 판정이다 |
