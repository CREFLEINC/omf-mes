...
| `CD-NOTIFICATION-TARGET-TYPE` | `EQUIPMENT` `MOLD` `INSTRUMENT` `PURCHASE_ORDER` `INTEGRATION_SYNC` `APPROVAL_REQUEST` `LOT` `WORK_ORDER` `NONCONFORMANCE` | `NOTIFICATION_TARGET_TYPE` | `targetTypeCode` | `registry-system` | 1 | 알림 대상 유형. ⛔ **시스템 소유** — 값이 우리 계약의 «대상 표 이름»이라 고객이 늘릴 수 없다(`A-10` 다형 참조 · `A-16`). ⚠ `LOT`·`WORK_ORDER`·`NONCONFORMANCE` 는 대응 이벤트가 알림센터 목록에 아직 없다. ⚠ 대응표에 없는 유형은 화면이 「대상으로 이동」을 열지 않는다 |
