...
| `CD-CYCLE-TYPE` | `DAY` `WEEK` `MONTH` `YEAR` | `CYCLE_TYPE` | `pmCycleUnitCode` `calibrationCycleTypeCode` | `registry-system` | 6 | 기간 단위. ⭐ **반례** — 아래 참조. ⛔ **값을 사전에 적자 결손이 드러났다**(2026-09-02) — `pmCycleUnitCode` 산문이 「일 또는 월」 **2값**으로 낡아 있었다. `#188` 이 4값으로 합쳐 확정했는데 형제 자리만 따라갔다. ⭐ **소유 = `registry-system` 확정**(사용자 결정 2026-09-02) — 공유계약 G-32 등록부 표와 같다 |
...
| `CD-EXTERNAL-SYSTEM` | `EQUIPMENT_STANDARD_IF` `TRACKING_SYSTEM` `UNIERP` | — | `externalSystemCode` | `enum` | 4 | 계약이 `enum` 으로 닫은 값 — `InterfaceDefinition` · `InterfaceDefinitionCreate`(`mdm-기준정보`) |
