...
- ⛔ **사람이 읽는 공지 번호는 두지 않는다.** 개념데이터모델(`design/wiki/domain-workflow/2026-07-03-개념데이터모델-v2-요구사항통합.md`)의 공지 행 «식별자(키)» 열에 `Notice No` 가 있으나, **그 열의 표기는 사람이 읽는 번호인지를 가르지 못한다** — 그 열은 엔티티의 키 칸이고, 사람이 읽는 번호를 두기로 한 것도 거기서는 `보전지시 No [확인필요]` 처럼 똑같이 적힌다. ⭐ **가르는 것은 «화면이 그 번호를 항목으로 갖는가»다** — 사람이 읽고 부르는 번호는 화면 필드표에 `business_no_t` 로 올라오고 채번 소관까지 지정된다(28화면 · 예: 보전지시번호 → `W-05-05` §5 「MES 단독 채번 · `app.numbering_rule` 소관」 · 출하번호 → `W-04-04` §5). **공지 화면(`W-CO-04`)에는 그 항목이 0건이다** — 목록 목업(§4)에도 작성 폼(§5-A)에도 없다. ⭐ 그리고 그 행이 인용한 확정(`결정 02`, 2026-07-06 · `design/raw/decisions/2026-07-06-설계결정서-14건.md`)의 속성 목록에도 번호가 없다 — 「제목·본문·게시 기간·작성자 + 대상 범위 + 표시 방식 + 수신자별 확인 이력」. 공지는 출력물 계약의 대상도 아니다(`06-API-요구서-app공통출력물.md` — 문서 유형 9종에 공지 없음). ⇒ `Notice No` 는 **엔티티를 가리키는 키**이고 그 형식·길이는 데이터 모델·구현 소관이다 — 설계가 채번 규칙을 세울 자리가 아니다.
