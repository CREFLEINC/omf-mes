...
| ⭐ **단말 상태 표시명** | **`GET /mdm/code-values?codeGroupCode=TERMINAL_STATUS`** — ⛔ **계약은 `statusCode` 만 내리고 표시명을 안 내린다.** 값 = `RUNNING`·`STOPPED` 둘뿐(**2026-09-02 신설** · `W-CO-06` §3 목업). ⚠ `isActive` 와 다른 축이다 | G-32 · K-5 |
| ⭐ **단말 유형 선택지·표시명** | **`GET /mdm/code-values?codeGroupCode=TERMINAL_TYPE`** — ⛔ 계약은 코드만 내리고 표시명을 안 내린다. 값 = `POP`·`MOBILE`(**2026-09-02 등재**) | G-32 · K-5 |
