...
- ⛔ **사람이 읽는 공지 번호는 두지 않는다.** 개념데이터모델(`design/wiki/domain-workflow/2026-07-03-개념데이터모델-v2-요구사항통합.md`)의 공지 행에 `Notice No` 가 있으나 그것은 **«식별자(키)» 열의 내부 식별자**이고, 같은 열의 `알림 No`·`홀드이력 No`·`등록 No` 와 같은 부류다 — 사람이 읽는 번호는 `생산LOT No` 처럼 **채번규칙이 함께 명시**되고 대리키 논쟁이 있던 자리만 `(surrogate)` 가 붙는데, `Notice No` 는 둘 다 없다. ⭐ 결정적으로 **그 행이 인용한 확정(`결정 02`, 2026-07-06)의 속성 목록에 번호가 없다** — 「제목·본문·게시 기간·작성자 + 대상 범위 + 표시 방식 + 수신자별 확인 이력」. 화면도 번호 열을 두지 않고(`W-CO-04` §3) 공지는 출력물 계약의 대상이 아니다(`06-API-요구서-app공통출력물.md` — 문서 유형 9종에 공지 없음).
