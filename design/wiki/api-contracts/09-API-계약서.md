...
⭐ **판정은 위 명령이 한다** — 화면 ID(`W-06-02 §4-A`) · 조항 번호(`공유계약 B-1`) · 이슈 번호(`omf-mes#147`)는 **잡지 않는다.** 설계 문서 경로를 지울 때 이슈 번호로 대체하는 것이 규약이 지정한 처방이다(`check-public-safe.py` 의 규칙 **「설계 문서 경로」·「설계 진행 상태」·「사내 운영 용어」**). ⛔ **행 번호로 인용하지 않는다** — 그 파일이 한 줄만 늘어도 낡는다(2026-09-06 실제로 그렇게 낡았다).
