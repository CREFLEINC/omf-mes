...
>
> ⭐ **이 대장은 «설계팀 자신»을 위한 것이다** — 우리 설계의 미결이 화면 스펙 113장에 흩어져 있어,
...
⛔ **차단 4건 — 그 화면은 골격이 바뀔 수 있다.**

살아 있는 미결이 **306행**이지만 그중 **화면 골격을 바꾸는 것은 4건**이다.
...
| --- | :-: |
| 화면 스펙 | **113** |
| └ 폐지 확정으로 제외한 스펙 | 5 — `P-02-05` · `P-02-06` · `P-02-07` · `P-04-02` · `W-06-13` |
| 미결 행 | **714** |
| 살아 있는 행 | **306** |
| 등급 열을 가진 행 | **443** |
| 그중 차단 | **4** |
| 살아 있는 행 중 **추적 표지가 없는 것** | **198 (65%)** |
...
> **기계적 기준**으로 뽑은 **47종**을 그대로 「확정되지 않은 업무 코드」로 실었는데, 그중 **45종은 이미 판정이 끝나 있었다.**
...
<details><summary>「코드 그룹이 아니다」로 판정된 6종 — 이유</summary>
...
| `serialControlTypeCode` | 코드 그룹을 세우지 않는다 — ⛔ 지금 업무에 시리얼 관리 «유형»을 가르는 축이 없다 «(정합주: 2026-09-10 — 구표기 「지금 업무에 «시리얼이 없다»」는 사실과 어긋난다 |
...
| 이름 | 사전 키(그룹 · 소유) |
| --- | --- |
| `causeCode` | `CD-EQUIPMENT-BREAKDOWN-CAUSE`(`EQUIPMENT_BREAKDOWN_CAUSE` · `registry`) |
