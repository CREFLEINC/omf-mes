...
⭐⭐ **이 저장소는 공개다. 그리고 보안 제약은 없다**(2026-09-06 사용자 확정). 실측 —
`gh api repos/CREFLEINC/omf-mes` → `"private": false` · **생성 시점(2026-06-27)부터 공개**다.
⚠ 하네스가 오래 「PRIVATE」로 적고 있었는데 그 기재는 **측정된 적이 없었다**(2026-09-03 「실측」
명령이 `omf-mes-server` 하나만 조회했다). 사용자 확정 원문 — 「공개가 의도된 것이다. 자료들에
보안성이 높은 것이 없고 프로젝트 자체가 보안성을 크게 필요로 하지 않는다. 지금은 **개발 편의성이
가장 중요**하다. 앞으로도 보안성은 고려하지 않아도 좋다.」
⇒ ⛔ **«저장소가 공개라는 이유로» 내용을 감추지 않는다.** 「누가 볼지 모르니 빼자」는 판단을
하지 않는다 — 그것이 개발 편의를 깎는다.

⚠ **범위를 긋는다 — 이 결정이 «삼키지 않는» 것 다섯:**

| 그대로인 것 | 왜 |
| --- | --- |
| `x-internal-note` 를 쓰는 관행 | 감추기가 아니라 **API 표면 분리**다 — `description` 은 생성 타입 주석으로 복사되므로 설계 부기(결손·근거 번호)는 거기 두지 않는다. **작성 규칙 2 가 그것을 의무로 둔다** |
| 「설계 변동 공지에 자세한 내용을 적지 않는다」(V3 규칙 5) | 보안이 아니라 **워크플로** — 개발팀이 열어 볼 자리를 우리가 미리 골라 주지 않는다 |
| `design/raw/` 읽기 전용 | 보안이 아니라 **시점 고착본 보호** |
| **제품 자신의 접근 설계** — 권한·역할·응답 노출 범위 | 그건 «우리 문서의 가시성»이 아니라 **설계 결정**이다. 화면·계약이 정하던 대로 정한다 |
| **고객 소유 자료** — `design/raw/customer/` 원자료 · 계약에 박히는 실물 식별자(실 사번·실 LOT) | **우리 소유가 아니다** — 가시성을 정할 권한이 우리에게 없다. 보안이 아니라 **소유권**이다. 집행은 이미 서 있다: `.gitignore` 의 `design/raw/customer/*` 규칙이 원자료를 빼고(목록 `00-목록.md` 만 추적) `check-public-safe.py` 의 「실 사번 의심」·「실 LOT 번호 의심」 두 규칙이 계약에서 막는다 — ⇒ 합성값을 쓴다 |
...
| 2026-09-05 | ⭐ **`status:in-progress` 부착 시점을 「접수」에서 「착수」로 옮겼다**(사용자 확정). ① `design-request-intake` Phase 0' 의 이슈 생성 명령에서 이 라벨을 뺐다 — 자기 이슈는 `Agent : Consultant` 하나로 시작한다 ② `design-issue-resolution` Phase 0 에 부착 지점을 신설했다 — Architect 가 **착수하기로 했을 때** 붙이고, 읽어만 보고 멈출 것이면 붙이지 않는다 ③ 떼는 자리는 그대로 이슈를 닫을 때(Phase 6b)이되, **즉시 답변 회차는 애초에 붙지 않으므로 그 줄을 건너뛴다**는 단서를 달았다(없는 라벨을 떼면 `gh` 가 실패한다) ④ `team-issue-protocol` §0 표·§2 라벨 뜻·§6 상태 전이도·§9 운영 피드백을 함께 맞췄다 | `design-request-intake` · `design-issue-resolution` · `team-issue-protocol` · `CLAUDE.md` | 3역할 도입 전에는 접수·판정·반영을 **한 세션이 이어서** 했으므로 「접수 = 착수」가 사실이었다. 3역할로 갈라진 뒤에도 부착 시점만 관행으로 남아, **인계만 받아 둔 이슈까지 「진행 중」으로 보였다** — 실제로 이 회차에 `#444`·`#445`·`#448` 세 건이 아무도 손대지 않은 상태로 그 라벨을 달고 있었다. ⭐ 라벨이 거짓을 말하면 그 라벨로 「지금 무엇이 돌아가는가」를 볼 수 없다 — V3 규칙 4(하는 일의 투명한 공개)가 라벨에 기대는 바로 그 기능이 무너진다. 이미 붙은 3건은 **사용자 판단으로 그대로 둔다**(소급하지 않는다) |
| 2026-09-06 | ⭐⭐ **보안 제약 폐지 + 가시성 기재 정정**(사용자 확정). 원문 — 「공개가 의도된 것이다. 자료들에 보안성이 높은 것이 없고 프로젝트 자체가 보안성을 크게 필요로 하지 않는다. 지금은 **개발 편의성이 가장 중요**하다. 앞으로도 보안성은 고려하지 않아도 좋다.」 ① `CLAUDE.md` 머리말에 ⭐⭐ 단락 신설 ② `00-authoring-rules.md` 의 「**모든 산출물은 내부 대외비**이며 외부 서비스에 업로드·공유·게시하지 않는다」 **폐기**(구표기 정합주 보존) ③ `public-boundary.md` 를 «보안 문서»에서 **「공지 본문 경계」**로 재편 — 표는 이제 **공지 본문에만** 적용한다. 우리 저장소에 무엇을 쓰나는 «개발 편의»가 정한다 ④ `team-issue-protocol` §1 재작성 — 가시성 갱신 + ⛔ **볼륨 수치를 표에서 걷고 측정 명령으로 대체**(세 번 낡았다) ⑤ `design-change-notice` SKILL §1 — 「`check-notice.py` 는 보안 검사가 아니라 **규칙 5** 검사」로 뜻을 바로잡음. ⚠ **V3 규칙 5(공지에 자세한 내용 금지)는 그대로다** — 보안이 아니라 「개발팀이 열어 볼 자리를 미리 골라 주지 않는다」는 워크플로 규칙이다 | `CLAUDE.md` · `00-authoring-rules.md` · `public-boundary.md` · `team-issue-protocol` §1 · `design-change-notice` SKILL §1 | PR `#457` 1차 리뷰가 ⛔ 6건을 냈고 그중 셋이 **내가 쓴 거짓 주장**이었다 — 「사흘 만에 뒤집혔다」(실제로는 **생성 시점부터 공개**다 — `PublicEvent` 는 전 이력에 1건이고 그 시각이 `created_at` 과 같다(`--paginate` 로 세야 보인다 · 첫 페이지만 보면 0 이 나온다)) · 「2026-09-06 재실측」이라 적고 볼륨 수치를 안 쟀다(이슈·OPEN·라벨 전부 불일치) · 「전수 실측 위반 0건」(정규식이 좁아 0을 냈다). ⭐ **뿌리는 「측정 안 하고 단정」** — 2026-09-03 에 「실측」이라 적힌 명령이 `omf-mes-server` 하나만 조회했고 `omf-mes` 는 **한 번도 측정된 적이 없었다.** 그리고 리뷰가 짚은 「경계를 저장소 전체로 넓히면 확정 마커 1,889곳이 소급 위반이 된다」는 **넓히는 방향 자체가 틀렸음**을 드러냈다 — 사용자 확정으로 그 방향을 철회한다. ⚠ **1,889 는 세는 법과 함께 읽는다** — `grep -rIn "✓확정\|✓설계확정" design/wiki/ \| wc -l`(마커가 든 «행» 수 · 2026-09-06 재현). 범위를 `design/` 로 넓히면 2,272 이고 `-o` 로 «출현» 수를 세면 더 크다 — 방법을 안 적으면 재현되지 않는 수다 |
