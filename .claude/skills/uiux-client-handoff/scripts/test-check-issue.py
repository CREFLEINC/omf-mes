...
`## 개발팀 전달사항 — <한 줄 결론>`(§7 템플릿 자신의 골격이고, 정본을 지킨 유일한
...
class ReplyHead(unittest.TestCase):
    """머리 표기 — 접두 일치이지 완전 일치가 아니다."""

    def test_민머리_통과(self):
        self.assertEqual(run("## 개발팀 전달사항" + BODY, "--reply", "--private"), 0)

    def test_한줄결론_붙은_형태_통과(self):
        """§7 템플릿 골격. 첫 판이 이것을 막았다 — 회귀 잠금."""
        head = "## 개발팀 전달사항 — 지적이 맞습니다. 스펙을 갱신합니다"
        self.assertEqual(run(head + BODY, "--reply", "--private"), 0)

    def test_206_실측_형태_통과(self):
        """정본을 지킨 유일한 실측 회신(omf-mes#206)의 첫 줄 그대로."""
        head = ("## 개발팀 전달사항 — 입력값은 «LOT 크기»가 맞습니다 · "
                "계약을 `lotSize` 필수로 교체했습니다")
        self.assertEqual(run(head + BODY, "--reply", "--private"), 0)

    def test_샵없음_차단(self):
        """omf-mes#232 실측 위반."""
        self.assertEqual(run("개발팀 전달사항" + BODY, "--reply", "--private"), 1)

    def test_구표기_차단(self):
        """omf-mes#222 실측 위반."""
        self.assertEqual(run("## 개발팀에 전달 사항" + BODY, "--reply", "--private"), 1)

    def test_접두만_같고_이어붙은_변형_차단(self):
        """접두 일치로 바꾸면서 헐거워지지 않았는지 — 공백으로 끊겨야 한다."""
        self.assertEqual(run("## 개발팀 전달사항입니다" + BODY, "--reply", "--private"), 1)
