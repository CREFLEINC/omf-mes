...
⚠ **부분 폐지 — 착수 통지 폐지(2026-09-03).** 이 파일에는 폐지된 착수 폼(기본 모드)을
검사하는 테스트가 섞여 있다. **지우지 않았다** — 그 테스트들이 잠그는 것은 폼 자체가 아니라
「검사기가 «막지 않아도 되는 것»을 막지 않는가」이고, 그 잠금이 풀리면 살아 있는 `--reply` ·
`--change-notice` 의 회귀 방어도 함께 헐거워진다. 새 기능을 착수 폼 위에 얹지 않을 뿐이다.
⭐ 지금 살아 있는 모드는 `--change-notice`(변경 요약 통지) 와 `--reply`(회신 규약)다.
