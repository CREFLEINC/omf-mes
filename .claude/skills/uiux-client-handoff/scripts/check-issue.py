...
무엇을 보나
  ...
  ③ 회신 규약 — 첫 줄이 「개발팀에 전달사항」으로 시작하는가, 「##」는 있어도 없어도 된다 (--reply 전용)
...
# ⛔ 완전 일치로 잡지 않는다. 뒤에 「 — <한 줄 결론>」을 붙여도 된다.
#    완전 일치로 두면 «규약을 지킨 사람»이 막히고, 막힌 사람은 검사기를 끈다.
# 뒤가 붙으려면 공백으로 끊겨야 한다 — 「개발팀에 전달사항입니다」 같은 변형은 막는다.
REPLY_HEAD = re.compile(r'^(?:##[ \t]+)?개발팀에 전달사항(?:[ \t]|$)')
    ...
    if reply:
        ...
        if not REPLY_HEAD.match(first):
            errs.append(('머리 표기', first[:60] or '(빈 줄)',
                         'team-issue-protocol §7 — 첫 줄은 「개발팀에 전달사항」으로 «시작»한다'
                         '(2026-08-27 재확정 — v2 원문 그대로, 「##」는 있어도 없어도 된다). '
                         '뒤에 「 — <한 줄 결론>」을 붙여도 된다'))
