...
⛔ 내용 금지 · ⛔ 백엔드/클라이언트 구분 금지. 규칙 N1~N6·T 가 그 모양을, 규칙 P 가 본문 경계를 본다.

⚠ P 는 «보안» 검사가 아니다 «(정합주: 2026-09-06 — 구표기 「규칙 P 가 **공개 안전**을 본다」.
이 프로젝트에는 보안 제약이 없다(사용자 확정) — P 가 막는 것은 유출이 아니라 **규칙 5** 다:
공지에 자세한 내용을 적어 개발팀이 열어 볼 자리를 미리 골라 주지 않는다)». 잡는 것이 무엇인지는
바뀌지 않았다 — 정규식은 그대로다.
...
------------------------------------------------
  ...
  ⛔ P  본문 경계 — 옛 check-issue.py 의 BLOCKING 10종 그대로(이미지·조항 요약·확정 기록 마커·
...
# ─── 본문 경계 — 옛 check-issue.py(BLOCKING 10 · ADVISORY 5 · PLACEHOLDER) 를 그대로 이식했다.
#     막는 근거는 규칙 5(자세한 내용 금지)다. 정규식·설명을 바꾸지 않았다 — 「무엇을 잡느냐」가 실측
