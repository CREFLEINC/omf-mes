...
description: >-
  ...
  요청에 반드시 이 스킬을 사용한다. ⛔ omf-mes-client로 내보내는 변경 요약 통지는
  uiux-client-handoff 소관이다(이 스킬은 그 마지막 단계에서 그 스킬을 호출만 한다).
  ⚠ 2026-09-03 개정 — 「착수 가능 통지」는 폐지됐다. 아웃바운드에 남는 것은 변경 통지뿐이다.
---
...
넣는다). ⚠ **표지가 0건이어도 그것이 「분석할 것이 없다」는 뜻은 아니다** — 표지는 권장일 뿐
의무가 아니라(Phase 8), 실측상 살아 있는 미결의 3분의 2에 표지가 없다. 분석 범위는 표지가
아니라 물음이 가리키는 화면으로 정한다.
...
⚠ 2026-09-03 개정 — 여기서 말하는 「상태」는 **우리 설계 문서의 상태**다. 프론트가 그 화면을
어디까지 만들었는지는 **우리가 보유하는 정보가 아니므로 브리핑에 적지 않는다.**
...
## Phase 7 — ★ 사람 게이트 (2/2) — 변경 요약 통지 발행

⚠ **2026-09-03 개정 — 여기서 내보내는 것은 「변경 요약 통지」 하나뿐이다.** 「착수 가능 통지」는
폐지됐다. 이 통지는 **무엇이 언제 바뀌었는지만** 알리고, 바뀐 내용 자체는 개발팀이 설계 자료를
직접 열람해 확인한다 — 어떻게 처리할지도 개발팀이 정한다. 이 게이트에서 처리 방법을 지시하지 않는다.
...
번호만 나열하지 말고 「무슨 화면인지 · 우리 설계가 지금 어떤 상태인지」를 함께 적는다
(⛔ 프론트의 구현 진도는 우리가 보유하는 정보가 아니다 — 2026-09-03 개정).

## Phase 6b — 닫기 (3중 잠금)

셋 다 통과해야 닫는다. 하나라도 실패하면 닫지 않고 사유를 기록한다.
...
⛔ **구 잠금 ④(`collect-open-items.py --issue <N>` 걸린 화면 0건)를 걷어냈다 —
2026-09-03 개정.** 그 판정이 읽는 「추적 표지」가 **살아 있는 미결 369행 중 244행(66%)에
없다.** 규약이 있는데 3분의 2가 안 따랐고, 2026-09-02 `#349` 에서는 판정 오류로 **살아 있는
미결 5행을 「해소」로 세어** 닫으면 안 되는 이슈가 닫힐 뻔했다. **안 지켜진 데이터에 건 게이트는
막지 못하는 것을 막는 척하면서 틀린 답으로 문을 연다.** 표지는 Phase 8 에서 권장으로만 남는다.

통과하면 `team-issue-protocol` §6 상태 전이에 따라 라벨을 정리하고 이슈를 닫는다. 닫을 수 없는
경우(백엔드 결정 대기 등)는 `status:in-progress`→`help wanted`로 교체 «(정합주: 2026-09-03 — 종전 `in progress` 였다. `in progress` 라벨은 신설이 취소됐고 기존 `status:in-progress` 를 쓰기로 확정돼 있었는데(CLAUDE.md 2026-08-25) 이 줄만 안 따라갔다 · `team-issue-protocol` §2)» + 사유 코멘트, 열어 둔다.

## Phase 8 — 되먹임

⭐ **추적 표지는 «권장»이다 — 2026-09-03 개정.** 반영한 화면의 스펙 §8에 이 이슈 번호(`#N`)를
심어 두면 미결-대장 역인덱스가 이 결정을 다시 찾아 준다. 붙이면 편하지만 **없다고 게이트가 막지
않는다**(Phase 6b 에서 그 잠금을 걷어냈다).

⚠ **왜 의무가 아닌가** — 표지는 「답이 오면 어디를 고칠지」를 **미리 적는 예측**이다. 개정된
업무 방식은 설계팀이 개발팀의 진행을 예측·보유하지 않고 **사후에 무엇이 언제 바뀌었는지만
전달**하는 쪽이다. 예측을 의무로 걸면 안 지켜지고, 안 지켜진 표지에 게이트를 걸면 틀린 답이 나온다.
...
P5b 병합 확인 → P6a 회신 게시 → P7 게이트(⚠ 변경 통지, 승인 후 발행) → P6b 3중 잠금 통과 →
닫힘 → P8 재생성(표지 심기는 권장).
