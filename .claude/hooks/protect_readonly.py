...
- 생성물(GENERATED_FILES) : design/wiki/progress/*.md 4종 + design/schema/generators/openapi/
...
# 생성물 — design/wiki/progress/*.md 4종
# ⚠ settings.json 의 deny 는 Write/Edit 만 막는다 — Bash 리다이렉션은 이 목록이 막는다.
#   변경-요약.md 가 빠져 있던 동안(2026-09-03) 그 경로로 실제 오염이 났다.
GENERATED_RELS = {
    Path("design/wiki/progress/화면-진도표.md"),
    Path("design/wiki/progress/미결-대장.md"),
    Path("design/wiki/progress/99-설계진도대장.md"),
    Path("design/wiki/progress/변경-요약.md"),
}
...
def check_bash(cmd, cwd):
    # 1) 리다이렉션 대상 (> design/raw/x, >> design/wiki/progress/화면-진도표.md ...)
